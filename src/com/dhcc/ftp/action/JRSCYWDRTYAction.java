...
public class JRSCYWDRTYAction extends BoBuilder implements ModelDriven<JrTy> {
	private JrTy po;
	...
	public String List() throws Exception {
		...
		String pageName = "JRSCYWDRTY_List.action?pageSize=" + pageSize;
		String hsql = "from JrTy where 1=1 ";
		...
		JrTyUtil = queryPageBO.queryPage(hsql, pageSize, page, rowsCount,
				pageName);
		request.setAttribute("JrTyUtil", JrTyUtil);
		return "List";
	}

	public String edit() throws Exception {

		String hsql = "from JrTy where acId ='" +po.getAcId() + "'";
		JrTy jrty = (JrTy) df.getBean(hsql, null);
		request.setAttribute("jrty", jrty);
	...
	public String del() throws Exception {
		...
		for (int i = 0; i < acIds.length; i++) {
			String hsql = "delete from JrTy where acId='" + acIds[i]+"'";
			df.delete(hsql, null);
		}
		return null;
	}
	...
	public String doImport() throws Exception {
		...
		try {
			List<Object> list = FtpUtil.importExcel(file, JrTy.class);
			for (Object obj : list) {
				JrTy jrty = (JrTy) obj;
				if (jrty == null) {
					// 待修改
				...
				if(null!=jrty.getStartDate()&&!jrty.getStartDate().equals("")&&jrty.getEndDate()!=null&&!jrty.getEndDate().equals("")){
					SimpleDateFormat sdFormat = new SimpleDateFormat("yyyyMMdd");
					sdFormat.format(new Date(Integer.parseInt(jrty.getStartDate())));
					Integer wholeDays = Integer.parseInt(jrty.getStartDate());
					Calendar calendar = new GregorianCalendar(); // using default time-zone
					setCalendar(calendar, wholeDays, 0, false);
					jrty.setStartDate(sdFormat.format(calendar.getTime()));
					wholeDays = Integer.parseInt(jrty.getEndDate());
					setCalendar(calendar, wholeDays, 0, false);
					jrty.setEndDate(sdFormat.format(calendar.getTime()));
				}else{
...
//					}
				...
				jrty.setAcId(jrty.getAcId()+"_JRTY");
					df.update(jrty);
					successNum++;
				} else {
					//break;
	...
	public String checkId() throws Exception{
		String hqlfind = "from JrTy where acId='" + po.getAcId()+"_JRTY" + "'";
