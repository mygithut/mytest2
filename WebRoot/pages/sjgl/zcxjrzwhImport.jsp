...
<script type="text/javascript" language="JavaScript1.2">
  ...
  //先检查是否有文件上传上来了
    ...
    //Form.action = "<%=request.getContextPath()%>/ZCXJRZWH_doImport.action";
    //Form.submit();
    parent.parent.parent.parent.openNewDiv();
 $.ajax({
			url:'<%=request.getContextPath()%>/ZCXJRZWH_doImport.action',
			type: "POST",
			dataType:'json',  
			data: {
  ...
  function doBack() {
	 window.location.href = "<%=request.getContextPath()%>/ZCXJRZWH_List.action";
  }
 </script>
</head>
...
<form action="" method="post">
 <table class="table" width="80%" align="center">
	<tr>
		<td height="30" colspan="2" align="right"><a href="<%=request.getContextPath()%>/doc/Bond_yield.xls" class="button">下载模板</a></td>
	</tr>
	<tr>
		<th width="36%">
