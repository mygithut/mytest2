...
<title></title>
		...
		<script type="text/javascript" src="<%=request.getContextPath()%>/ftp/pages/js/artDialog4.1.6/plugins/iframeTools.source.js"></script>
<style type="text/css">
     #loading,ol{
      font-size:14px;
      display:none;
...
<script type="text/javascript" language="JavaScript1.2">
  ...
  //先检查是否有文件上传上来了
   ...
   // Form.action = "<%=request.getContextPath()%>/SHIBORWH_doImport.action";
   // Form.submit();
   parent.parent.parent.parent.openNewDiv();
   $.ajax({
			url:'<%=request.getContextPath()%>/SHIBORWH_doImport.action',
			type: "POST",
			dataType:'json',  
			data: {
  ...
  function ok(){
     window.location.href ='<%=request.getContextPath()%>/distribute.action?url=SHIBORWH';
  }
  function doReload() {
	     window.location.reload();
	  }
	  function doBack() {
		 window.location.href = "<%=request.getContextPath()%>/SHIBORWH_List.action";
	  }
 </script>
</head>

<body>
<form action="<%=request.getContextPath()%>/SHIBORWH_doImport.action" method="post" id="doImport" name="doImport">
<%Integer successNum = (Integer)request.getAttribute("successNum");
 %>
<table class="table" width="80%" align="center">
	<tr>
		<td height="30" colspan="2" align="right"><a href="<%=request.getContextPath()%>/doc/Shibor.xls" class="button">下载模板</a></td>
	</tr>
	<tr>
		<th width="36%">
		<p align="right">选择Excel文件：</p>
		</th>
		<td width="64%"><input type="file" id="ClientExcelFile" name="ClientExcelFile" size="50"></td>
	</tr>
	<tr><td colspan="2"><div id="loading" align="center"></div></td></tr>
	<tr>
		<td height="30" colspan="2">
			<div align="center">
			<input type="button" name="Submit1" value="导&nbsp;&nbsp;入" onclick="my_check(this.form)" class="button"> &nbsp;&nbsp;
		    <input type="button" name="Reset" value="重&nbsp;&nbsp;置" class="button" onclick="doReload()">&nbsp;&nbsp;
		    <input type="button" name="Submit1" value="返&nbsp;&nbsp;回" onclick="doBack()" class="button"></div>
		</td>
	</tr>
</table>
<BR/>
