...
ol {
  ...
  //先检查是否有文件上传上来了
	  ...
	  if(o==null||o==""){
		...
		$.ajax({
			url:'/ftp/JRSCYWDRINVEST_doImport.action',
			type: "POST",
			dataType:'json',  
			data: {
  ...
  function ok(){
     window.location.href ='/ftp/distribute.action?url=JRSCYWDRINVEST';
  }
  function doReload() {
  ...
  function doBack() {
	...
	<body>
		<form action="/ftp/JRSCYWDRCJ_doImport.action" method="post"
			...
			<table class="table" width="80%" align="center">
				<tr>
					<td height="30" colspan="2" align="right">
						<a href="<%=request.getContextPath()%>/doc/invest.xls"
							class="button">下载模板</a>
					</td>
				</tr>
				<tr>
