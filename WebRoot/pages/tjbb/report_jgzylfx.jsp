...
function doQuery(){
	...
	var isMx = 0;
	window.frames.downFrame.location.href ='<%=request.getContextPath()%>/REPORT_jgzylfxReport.action?assessScopeText='+encodeURI(assessScopeText)+'&brName='+encodeURI(brName)+'&brNo='+brNo+'&tjType='+tjType+'&superBrNo='+brNo+'&manageLvl='+manageLvl+'&date='+date+'&assessScope='+assessScope.value+'&isMx='+isMx+'&isFirst=1';
...
function doRefresh() {
	...
	var url = "<%=request.getContextPath()%>/REPORT_clearCache.action";
	new Ajax.Request(url, {
		method : 'post',
		parameters : {
			brNo:brNo,manageLvl:manageLvl,date:date,tjType:tjType,assessScope:assessScope,t:new Date().getTime()
		},
		onSuccess : function() {
	    	   art.dialog({
             	    title:'成功',
